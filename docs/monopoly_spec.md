...
3) Полные правила “классической Монополии”, сделки разрешены (см. ниже), включая:
...
- data/rules.yaml: флаги правил, включая HR1, HR2, free_parking_empty=true, no_trades=false.
...
7) Сделки включены:
   - торговля разрешена между ботами; только активный игрок в конце хода делает 1 оффер;
   - ответ только accept/reject, без встречных предложений;
   - оффер может включать несколько лотов: деньги, собственности, get_out_of_jail_card;
   - собственность с домами/отелями (и любая в группе, где есть дома/отели) — не продаётся, сначала нужно продать постройки банку;
   - заложенная собственность может передаваться, новый владелец выбирает:
     - сразу выкупить ипотеку (платит mortgage + 10% и переворачивает карточку вверх), или
     - оставить в ипотеке (платит 10% сейчас; при последующем выкупе платит mortgage + 10% ещё раз);
   - если у получателя нет денег даже на 10% при передаче — сделка невалидна;
   - анти-спам: повторный оффер той же паре должен быть строго лучше (минимум +2% к последнему score), иначе отклоняется;
     сброс истории — если изменился набор собственности или net worth любого игрока > 30%.
...
- Сделки реализованы только по правилам выше; без усложнений (контр-офферы запрещены).
